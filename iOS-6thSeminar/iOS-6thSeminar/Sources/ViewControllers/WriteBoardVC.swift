...
class WriteBoardVC: UIViewController {
    ...
    @objc func completeWrite() {
        ...
        guard titleTextField.text?.isEmpty != true else {return}
        guard contentTextView.text?.isEmpty != true else {return}
        
        // 작성된 api를 사용하여 게시글을 작성하고 만약 완료되었다면 
        BoardService.shared.postBoard(title: gsno(titleTextField.text), contents: gsno(contentTextView.text), image: contentImage.image!) {
